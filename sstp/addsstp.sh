...
clear
source /var/lib/Gashakgeneration22store/ipvps.conf
if [[ "$IP2" = "" ]]; then
...
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${CLIENT_EXISTS} == '0' ]]; do
		...
		CLIENT_EXISTS=$(grep -w $user /var/lib/Gashakgeneration22store/data-user-sstp | wc -l)
...
$user * $pass *
EOF
echo -e "### $user $exp">>"/var/lib/Gashakgeneration22store/data-user-sstp"
clear
cat <<EOF
